...
class BiliCrawler:
    ...
    # ========== 新增：跨平台 FFmpeg 路径获取和子进程调用 ==========
    def get_ffmpeg_path(self):
        """获取当前平台下的 FFmpeg 可执行文件路径"""
        if sys.platform == "win32":
            ffmpeg_name = "ffmpeg.exe"
        else:
            ffmpeg_name = "ffmpeg"
        # 先检查打包资源目录
        bundled_path = self.resource_path(ffmpeg_name)
        ...
        raise FileNotFoundError("FFmpeg 未找到，请安装 ffmpeg 或将其放在程序目录下")

    def run_subprocess(self, cmd):
        """跨平台子进程调用（自动隐藏窗口仅限 Windows）"""
        if sys.platform == "win32":
...
class BiliCrawlerGUI(QMainWindow):
    ...
    def start_crawling(self):
        ...
        # 创建工作线程
        self.crawler_thread = CrawlerThread(self.crawler, url, save_dir, quality=quality, download_series=download_series,
                                            series_range=series_range, download_danmu=download_danmu,
